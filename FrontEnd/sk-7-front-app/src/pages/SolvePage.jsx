            ...
            >
                ...
                <div
                    ...
                    style={
                        isMobileView
                            ...
                            : { width: `${leftPanelWidth}%`, flex: "none" } // 데스크탑: 상태에 따른 너비
                    }
                >
                    {/* ProblemSection 자체가 내부 스크롤을 갖도록 설계 (p-4 h-full overflow-auto) */}
                ...
                {!isMobileView && (
                    <div
                        className="w-1.5 hover:w-2 bg-gray-600 hover:bg-blue-500 cursor-col-resize
                                   ...
                                   transition-colors duration-100 ease-in-out" // transition-all -> transition-colors
                        onMouseDown={handleMouseDownOnHorizontalResizer}
                        onTouchStart={(e) => {
                            // 간단한 터치 지원 (데스크탑 이벤트 핸들러 재사용)
                            if (!isMobileView)
                                handleMouseDownOnHorizontalResizer(e);
                        }}
                    >
                        {/* Visual cue for the resizer handle (optional) */}
                ...
                <div
                    className={`relative flex-1 bg-gray-700 
                                ${
                                    isMobileView
                                        ? "w-full min-h-[50vh] flex flex-col" // 모바일: 세로 분할, CodeEditorSection이 내부 레이아웃 관리
                                        : "h-full flex flex-col overflow-hidden"
                                }`} // 데스크탑: CodeEditorSection이 내부 레이아웃 관리
                >

...
const ChatComponent = forwardRef(
    (
        {
            p_id = null, // 방 생성 시 필요, 참여 시에는 null
            lang = null, // 방 생성 시 필요, 참여 시에는 null
        },
        ref
    ) => {
        // 코드 상태 (Global)
        ...
        const { isLoggedIn, userName, userEmail } = useAuthStore();
                        ...
                        (message) => {
                            try {
                                ...
                                if (sender !== currentUserEmail) {
                                    // 내가 보낸 업데이트는 알림 X
                                    setEditingBy(sender.split("@")[0]); // 이름 또는 이메일 앞부분
        ...
        const subscribeInitialSetting = useCallback(
            (roomId) => {
                if (!stompClientRef.current?.active || !roomId) {
                    console.warn(
                        "초기 설정 구독 불가 (S03): 클라이언트 비활성 또는 방 ID 없음."
                    );
                    return;
                }
                ...
                initialSettingsSubscriptions.current.members =
                    stompClientRef.current.subscribe(
                        ...
                        (message) => {
                            try {
                                const { members, owner } = JSON.parse(
                                    message.body
                                );
                                console.log(
                                    "멤버 목록 수신:",
        ...
        const connectToHelpRoom = useCallback(async () => {
            ...
            // `currentUserEmail`은 로그인 상태 변경 useEffect에서 이미 설정됨
            ...
            // STOMP 클라이언트가 이미 활성화된 경우 (예: 이전 방에서 나온 직후)
            if (stompClientRef.current?.active) {
                console.log(
                    "STOMP 클라이언트 이미 활성 상태. 새 방 생성 시도."
                );
                if (!currentRoomId) {
                    // 현재 참여 중인 방이 없을 때만 새 방 생성
                    setConnectionStatus("도움방 생성 중...");
                    try {
                        // TODO : 초기 코드 추가 (code: )
                        ...
                        if (roomData?.id) {
                            console.log(
                                "기존 STOMP 연결 위에서 새 방 생성 성공! Room ID:",
                                roomData.id
                            );
                            // currentRoomId 변경 -> useEffect가 새 방에 대한 구독 처리
                            setCurrentRoomId(roomData.id);
                        } else {
                            setConnectionStatus("방 생성 실패 (ID 없음)");
                        }
                    } catch (error) {
                        setConnectionStatus("방 생성 실패 (요청 오류)");
                        console.error(error);
                    }
                } else {
                    console.log(
                        "이미 방에 참여 중입니다. (currentRoomId:",
                        currentRoomId,
                        ")"
                    );
                }
                return; // 추가적인 STOMP 연결 생성 방지
            }
            ...
            const client = new Client({
                ...
                onConnect: async (frame) => {
                    ...
                    setIsConnected(true);
                    // 연결 성공 후 방 생성
                    setConnectionStatus("도움방 생성 중..."); // 상태 명확히
                    try {
                        const roomData = await createRoom({
                            problemId: p_id,
                            language: lang,
                        ...
                        if (roomData?.id) {
                            console.log(
                                "STOMP 연결 후 HTTP 방 생성 성공! Room ID:",
                                roomData.id
                            );
                            setCurrentRoomId(roomData.id); // useEffect가 구독 처리
                        } else {
                            setConnectionStatus("방 생성 실패 (ID 없음)");
                            disconnectFromHelpRoom(); // 연결은 됐으나 방 생성 실패 시 연결 해제
                        }
                    } catch (error) {
                        setConnectionStatus("방 생성 실패 (요청 오류)");
                        console.error(error);
                        disconnectFromHelpRoom(); // 에러 발생 시 연결 해제
                    }
                },
                onStompError: (frame) => {
                    console.error(
        ...
        // 권한 변경
        const changeUserRole = (targetUserEmail, newRole) => {
            if (
                stompClientRef.current?.active &&
                currentRoomId &&
                currentUserEmail === ownerEmail
                ...
                stompClientRef.current.publish({
                    destination: `/app/room.changeRole.${currentRoomId}`,
                    body: JSON.stringify({
                                                    ...
                                                    email !==
                                                        ...
                                                        <>
                                                            <select
                                                                ...
                                                                className="bg-gray-600 text-xs p-0.5 rounded border border-gray-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                                                                title="역할 변경"
                                                            >
                                                                <option value="CHAT_ONLY">
                                                                    채팅만
                                                                </option>
                                                                <option value="CHAT_AND_EDIT">
                                                                    코드편집
                                                                </option>
                                                            </select>
                                                            {/* <button
